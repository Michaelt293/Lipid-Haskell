...
spec = do
  describe "Test for quasiquoters and Shorthand instances" $ do
    ...
    it "QuasiQuoter for TG 52:2" $
      shorthand @ (TG (Maybe DeltaPosition)) [tgMaybeDelta|TG 52:2(9Z,12Z)|] `shouldBe` "TG 52:2(9Z,12Z)"
    it "QuasiQuoter for TG 52:2" $
      shorthand @ (TG DeltaPosition) [tgDelta|TG 52:2(9Z,12Z)|] `shouldBe` "TG 52:2(9Z,12Z)"
    it "QuasiQuoter for DG 34:0" $
  ...
  describe "Test for quasiquoters and NNomenclature instances" $ do
    ...
    it "QuasiQuoter for TG 52:2" $
      nNomenclature @ (TG (Maybe OmegaPosition)) [tgMaybeOmega|TG 52:2(n-6,n-9)|] `shouldBe` "TG 52:2(n-6,n-9)"
    it "QuasiQuoter for TG 52:2" $
      nNomenclature @ (TG OmegaPosition) [tgOmega|TG 52:2(n-6,n-9)|] `shouldBe` "TG 52:2(n-6,n-9)"
    it "QuasiQuoter for DG 34:0" $
